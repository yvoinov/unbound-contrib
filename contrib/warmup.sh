...
dig_options="+short +time=5 +tries=1"

built_in ()
{
echo "Warming up cache by MRU domains..."
$dig $dig_options -f - @$address >/dev/null 2>&1 <<EOT
2gis.com
2gis.kz
2gis.ru
admir.kz
...
if [ ! -z "$1" ]; then
 ...
 if [ -f $1 ]; then
  $cat $1 | $dig $dig_options -f - @$address >/dev/null 2>&1 
 else
  echo "File $1 not found, using built-in list..."
  built_in
 fi
else
 built_in
